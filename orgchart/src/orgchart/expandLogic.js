...
export function expandirFantasmas(chart, lineaFiltro) {
...
export function postCargaConReintento(chart, { antonioId, corporativoExpandido, lineaFiltro }, intentosRestantes = 6) {
  try {
    if (corporativoExpandido && antonioId) {
      ...
      if (antonioNode && antonioNode.childrenIds && antonioNode.childrenIds.length > 0) {
        chart.expandCollapse(antonioId, antonioNode.childrenIds, [], function () {
          expandirFantasmas(chart, lineaFiltro);
        });
      } else {
        expandirFantasmas(chart, lineaFiltro);
      }
    ...
    if (intentosRestantes > 0) {
      setTimeout(
        () => postCargaConReintento(chart, { antonioId, corporativoExpandido, lineaFiltro }, intentosRestantes - 1),
        75,
      );
    } else {
      try {
